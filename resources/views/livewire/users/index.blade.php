<div>
    ...
    <div class="mb-4 flex items-center justify-between gap-2">
        ...
        <flux:button href="{{ route('users.create') }}" wire:navigate variant="primary" color="blue" size="sm"
            icon="plus">
            Crear registro
        </flux:button>
    </div>

    <flux:input wire:model.live="search" icon="magnifying-glass" placeholder="Buscar usuario ..." class="mb-4" />
                ...
                <tbody class="divide-y divide-outline dark:divide-outline-dark">
                    @forelse ($users as $user)
                        <tr class="border-t">
                            ...
                            <td class="p-2">
                                <flux:link class="text-info" href="{{ route('users.show', $user) }}">
                                    {{ $user->name }}
                                </flux:link>
                            </td>
                            ...
                            <td class="p-2">
                                @switch($user->role)
                                    @case('root')
                                        <flux:badge rounded icon="user" color="red">
                                            Super usuario
                                        </flux:badge>
                                    @break

                                    @case('admin')
                                        <flux:badge rounded icon="user" color="blue">
                                            Administrador
                                        </flux:badge>
                                    @break

                                    @case('user')
                                        <flux:badge rounded icon="user" color="gray">
                                            Usuario
                                        </flux:badge>
                                    @break
                            ...
                            <td class="px-4 py-2">
                                <flux:button href="{{ route('users.show', $user) }}" wire:navigate variant="primary"
                ...
                <div
                    ...
                    <div class="flex items-start justify-between gap-2">
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <flux:link class="text-info font-medium" href="{{ route('users.show', $user) }}">
                                {{ $user->name }}
                            </flux:link>
                        </div>
